...
{
    const int Pcharacter = 0;
    const int EqSaveData = 1;
    static public void SavePartyButtonPressed()
    {
        // Get the directories currently on the C drive.
        // DirectoryInfo[] cDirs = new DirectoryInfo(@"C:\Users\User\Downloads\StreamingDataLab-main\StreamingDataLab\Assets").GetDirectories();

        using StreamWriter sw = new StreamWriter(Application.dataPath + Path.DirectorySeparatorChar + "JinData.txt");
    
        foreach (PartyCharacter pc in GameContent.partyCharacters)
        {
            Debug.Log("PC class id == " + pc.classID);
            
            sw.WriteLine(Pcharacter + "," + pc.classID + "," + pc.health + "," + pc.mana + "," + pc.strength + "," + pc.agility + "," + pc.wisdom);

            foreach (int equipID in pc.equipment)
            {
                sw.WriteLine(EqSaveData + "," + equipID);
    ...
    static public void LoadPartyButtonPressed()
    {
        ...
        StreamReader sr = new StreamReader(Application.dataPath + Path.DirectorySeparatorChar + "JinData.txt");
        string line;
        ...
        while ((line = sr.ReadLine()) != null)
        {
            ...
            if (saveDataSignifier == Pcharacter) 
            {
            ...
            else if (saveDataSignifier == EqSaveData)
            {
